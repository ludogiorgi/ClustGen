...
b2half = b^2/2
x0 = [1.0]
sol = evolve(x0, forcing, 100000, b) # It works well also with 100000 timesteps
U(x) = - x^2/2 + x^4/4
...
plt = plot()
for i in 1:500
    plot!(plt, 1/n_diffs:1/n_diffs:diff_stop/n_diffs, ens_f[i,:], color=:red, legend=false, alpha=0.1)
end
display(plt)
# savefig(plt, "diff_trj.png")
##
...
#scatter!(centers_values[index][:], averages_true, color=:red)
scatter!(centers_values[index][:], .- averages_values[index][:], color=:red)
savefig("f_true.png")
##
rmse = 0.0
for i in 1:n_diffs
...
function train_vanilla2(obs, n_epochs, batch_size, neurons, m, σ2; opt=ADAM(0.001), activation=swish, ϵ=0.05)
    nn_temp = create_nn(neurons, activation=activation)
    nn = Chain(nn_temp, x -> x .* 3)
    losses = []
    μ = obs'
    Dim = size(obs)[1]
    losses = []
...
nn_vanilla, loss_vanilla = train_vanilla2(obs[:,1:10:end], 200, 64, [Dim+1, 64, 64, Dim], m, σ2; activation=tanh)
...
# function evolve_r(x1, n_diffs, f, g, score, diff_stop)
#     dt = 1.0 / n_diffs
...
#         x_old = x_r[:, i-1]
#         x_r[:, i] = x_old .+ f(t_diff) .* (x_old .- 0.5) .* dt .+ score(x_old, t_diff) .* g(t_diff).^2 .* dt .+ g(t_diff) .* randn(1) .* sqrt(dt)
#     end
#     return x_r[:, 1:end]
# end
...
# for i in ProgressBar(1:n_ens)
#     start_r = [STD*randn()+0.5]
#     ens_r[i,:] = evolve_r(start_r, n_diffs, f, g, score_clustered, diff_stop)
#     #ens_r_vanilla[i,:] = evolve_r([variance_exploding(1)*randn()], n_diffs, g, variance_exploding, nnv, n_diffs)
# end
# ##
# kde_r = kde(ens_r[:,end])
# plt = plot(kde_r.x, kde_r.density, color=:red, label="Clustered")
# ##
# tax = [(n_diffs - i + 1) / n_diffs for i in 1:diff_stop]
# plt = plot()
# for i in 1:100
#     plot!(plt,tax, ens_r[i,:], color=:blue, legend=false, alpha=0.3)
# end
# plt
# ##
...
# forward_ens = zeros(n, diff_stop)
# for i in 1:n
#     start_f = [mt[end]]
#     forward_ens[i,:] = evolve_f(start_f, n_diffs, f, g, diff_stop)
# end
# forward = mean(forward_ens, dims=1)
...
# ##
# sc(x,t) = score_clustered(x,t)[1] 
# plot(tax, sc.(1, tax), color=:red, label="Clustered")
# plot!(tax, f.(tax), color=:blue, label="True")
