...
function compute_moments_from_simulation(p, observables; nsteps_sim=1_000_000, dt=0.01, res=10)
    ...
    obs .= (obs .- M) ./ S  # Normalize the observations
...
μ_obs = mean(obs, dims=2); μ₁, μ₂, μ₃ = μ_obs[1],μ_obs[2],μ_obs[3]
...
initial_moments = [mean(observable(x) for x in eachcol(obs)) for observable in observables]

# --- Step 3: Compute R_linear and J_numerical ---
println("\n--- Computing R_linear ---")
R_linear = create_response_matrix(obs, 0.1, observables, create_conjugate_observables(linear_score(obs)))

println("\n--- Computing R_score ---")
R_score = create_response_matrix(obs, 0.1, observables, create_conjugate_observables(score_clustered))

sim_args_fast = (nsteps_sim=100_000_000, dt=0.01, res=10) 
J_numerical = compute_numerical_jacobian(params, params_to_control, observables; ε=1e-4, sim_args_fast...)
...
println("\n➡️  Response Matrix R (from KGMM Score Approximation):"); display(round.(R_score, digits=4))
...
# Run 2: Using the Linear Score Response Matrix
println("\n--- Analyzing with R_linear ---")
predicted_Rs, actual_Rs, params_hist_Rs = run_sensitivity_analysis(R_score, params, params_to_control, initial_moments, observables;
                                             n_control=5,
                                             base_Δμ=fill(-0.02, 3),
                                             λ=0.001,
                                             sim_args_fast...)                                             
...
if @isdefined(predicted_J) && @isdefined(actual_J)
    ...
    for i in 1:3
        ...
        p = plot(predicted_J[i, :], actual_J[i, :], label="Jacobian", color=:purple, markersize=5, markerstrokewidth=0.5,
                    xlabel="Predicted Change", ylabel="Actual Change", title=moment_labels[i],
                    legend=:bottomright)
        plot!(p, predicted_R[i, :], actual_R[i, :], label="Linear Score (R)", color=:green, markersize=5, markerstrokewidth=0.5)
        plot!(p, predicted_Rs[i, :], .-actual_Rs[i, :], label="KGMM Score (R)", color=:red, markersize=5, markerstrokewidth=0.5)
...
if @isdefined(params_hist_J) && @isdefined(params_hist_R)
    ...
    for param_idx in 1:length(params_to_control)
        for moment_idx in moments_to_plot_indices
            ...
            plot!(p, target_moment_changes, params_hist_Rs[param_idx, :],
                  label="KGMM Score (R)",
                  lw=2, color=:red)
