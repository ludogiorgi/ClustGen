...
function train_kgmm_score_from_data(
    ...
    epochs::Int = 2000,
    batch_size::Int = 32,
    nn_architecture::Vector{Int} = [1, 100, 50, 1],
    activation = swish,
    ...
    # Determine which data to use for clustering based on the normalization flag
    data_for_clustering = use_normalization_for_clustering ? obs : raw_timeseries
    averages, centers, Nc,_ = f_tilde_ssp(σ_value, data_for_clustering; prob=clustering_prob, do_print=true, conv_param=clustering_conv_param, normalization=use_normalization_for_clustering)
    ...
    nn_clustered, loss_clustered = train(inputs_targets, epochs, batch_size, nn_architecture; use_gpu=use_gpu, activation=activation, last_activation=last_activation)

    # Post-process the NN and move it to the CPU
    ...
    # === Define and Return the Final Score Function ===
    final_score_function(x) = .- nn_clustered_cpu(Float32.([x...]))[:] ./ σ_value

    function score_clustered_nn(x)
        return unnormalize_f(final_score_function, x, M, S)
    end

    # Return all useful artifacts in a NamedTuple
    return (
        score_function = score_clustered_nn,
        trained_nn = nn_clustered_cpu,
        loss_history = loss_clustered,
        normalization_factors = (M, S)
    )
end

kgmm_results = train_kgmm_score_from_data(obs_nn;
                                            epochs=1000,
...
##
# #################### LOADING THE SAVED NN ####################
# # To load the saved neural network (safe method):
# σ_value = 0.05
# loaded_nn = load_model_safe("NNs/nn_reduced_add")

# # Define score_clustered to handle both scalar and vector inputs
# function score_clustered(x)
#     if isa(x, Number)
#         # Handle scalar input by converting to vector
#         return (.- loaded_nn(Float32.([x]))[:] ./ σ_value)[1]
#     else
#         # Handle vector input
#         return .- loaded_nn(Float32.(x))[:] ./ σ_value
#     end
# end
...
# function create_response_matrix(time_series, dt, observables, conjugate_observables; 
#                                 max_lag_time=30.0,
#                                 enforce_t0_identity::Bool=false,
#                                 param_jacobians::Union{Nothing, Vector{<:Function}}=nothing,
#                                 observable_gradients::Union{Nothing, Vector{<:Function}}=nothing)
...
#     R = zeros(N_moments, N_params)
#     if enforce_t0_identity
#         R0 = zeros(N_moments, N_params)
#     end
#     Rt = zeros(N_moments, N_params, length(lag_indices))
...
#                 J_func = param_jacobians[i]
#                 grad_g_func = observable_gradients[k]
                
#                 # Calculate the theoretical value ⟨∇g ⋅ J⟩ from data
#                 # This handles both scalar and vector cases via the dot product
#                 inner_products = [dot(grad_g_func(x), J_func(x)) for x in time_series]
#                 R0[k, i] = mean(inner_products)
#             end
#         end
#     end
#     if enforce_t0_identity
#         C = pinv(Rt[:,:,1]) * R0
#         for i in 1:length(lag_indices)
#             Rt[:,:,i] = Rt[:,:,i] * C
#         end
#     end
#     return R, Rt, lags
        ...
        if recalculate_jacobian && iter > 1
            ...
            elseif jacobian_method == :response_matrix
                ...
                elseif score_type == :clustered
                    new_kgmm_results = train_kgmm_score_from_data(new_obs;
                                            epochs=1000,
                                            use_normalization_for_clustering=true)
...
observable_gradients = [
    x -> [1.0],                        # ∇g₁(x) = 1
    x -> [2 * (x[1] - M_obs)]           # ∇g₂(x) = 2(x - μ)
]

# --- 2. Define Parameter Jacobians ---
# For F(x) = a*x + b*x² - c*x³ + F̃, the parameters to control are [a, b, c, F̃]
param_jacobians = [
    x -> [x[1]],                       # J_a(x) = ∂F/∂a = x
    x -> [x[1]^2],                     # J_b(x) = ∂F/∂b = x²
    x -> [-x[1]^3],                    # J_c(x) = ∂F/∂c = -x³
...
common_kwargs = (D_eff=D_eff, n_control=15, base_Δμ= .- [0.01, 0.01], λ=0.0001)
...
save("combined_parameter_control_analysis_makie.pdf", fig_pdf)
...
data_file = joinpath(data_dir, "parameter_control_analysis_data.h5")
data_file = joinpath(data_dir, "reduced_add_2m.h5")
...
# data = load_parameter_control_data("scripts/ParameterControl/PC_data/reduced_add_2m.h5")
